...
// Default value is mvec(0.0f, 0.0f).
//   X
...
bool    ImGui::BeginTable(const char* str_id, int columns_count, ImGuiTableFlags flags, const mvec& outer_size, float inner_width)
{
    ImGuiID id = GetID(str_id);
    return BeginTableEx(str_id, id, columns_count, flags, outer_size, inner_width);
}

bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const mvec& outer_size, float inner_width)
{
    ImGuiContext& g = *GImGui;
    ...
    const mvec avail_size = GetContentRegionAvail();
    mvec actual_outer_size = CalcItemSize(outer_size, ImMax(avail_size.x, 1.0f), use_child_window ? ImMax(avail_size.y, 1.0f) : 0.0f);
    ...
    if (use_child_window)
    {
        ...
        mvec override_content_size(FLT_MAX, FLT_MAX);
        ...
        if (override_content_size.x != FLT_MAX || override_content_size.y != FLT_MAX)
            SetNextWindowContentSize(mvec(override_content_size.x != FLT_MAX ? override_content_size.x : 0.0f, override_content_size.y != FLT_MAX ? override_content_size.y : 0.0f));

        // Reset scroll if we are reactivating it
        if ((table_last_flags & (ImGuiTableFlags_ScrollX | ImGuiTableFlags_ScrollY)) == 0)
            SetNextWindowScroll(mvec(0.0f, 0.0f));
    ...
    inner_window->DC.PrevLineSize = inner_window->DC.CurrLineSize = mvec(0.0f, 0.0f);
...
void    ImGui::EndTable()
{
    ...
    const mvec backup_outer_max_pos = outer_window->DC.CursorMaxPos;
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    if (is_visible)
    {
        ...
        if (border_col && bg_y1 >= table->BgClipRect.Min.y && bg_y1 < table->BgClipRect.Max.y)
            window->DrawList->AddLine(mvec(table->BorderX1, bg_y1), mvec(table->BorderX2, bg_y1), border_col, border_size);

        // Draw bottom border at the row unfreezing mark (always strong)
        if (draw_strong_bottom_border && bg_y2 >= table->BgClipRect.Min.y && bg_y2 < table->BgClipRect.Max.y)
            window->DrawList->AddLine(mvec(table->BorderX1, bg_y2), mvec(table->BorderX2, bg_y2), table->BorderColorStrong, border_size);
    }
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    if (g.IO.KeyShift)
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            ...
            mvec text_pos = merge_group->ClipRect.Min + mvec(4, 4);
            mvec text_size = CalcTextSize(buf, NULL);
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            if (draw_y2 > draw_y1)
                inner_drawlist->AddLine(mvec(column->MaxX, draw_y1), mvec(column->MaxX, draw_y2), col, border_size);
        }
    }
    ...
    if (table->Flags & ImGuiTableFlags_BordersOuter)
    {
        ...
        {
            inner_drawlist->AddLine(outer_border.Min, mvec(outer_border.Min.x, outer_border.Max.y), outer_col, border_size);
            inner_drawlist->AddLine(mvec(outer_border.Max.x, outer_border.Min.y), outer_border.Max, outer_col, border_size);
        }
        else if (table->Flags & ImGuiTableFlags_BordersOuterH)
        {
            inner_drawlist->AddLine(outer_border.Min, mvec(outer_border.Max.x, outer_border.Min.y), outer_col, border_size);
            inner_drawlist->AddLine(mvec(outer_border.Min.x, outer_border.Max.y), outer_border.Max, outer_col, border_size);
        }
    }
    if ((table->Flags & ImGuiTableFlags_BordersInnerH) && table->RowPosY2 < table->OuterRect.Max.y)
    {
        ...
        if (border_y >= table->BgClipRect.Min.y && border_y < table->BgClipRect.Max.y)
            inner_drawlist->AddLine(mvec(table->BorderX1, border_y), mvec(table->BorderX2, border_y), table->BorderColorLight, border_size);
    }
...
void ImGui::TableHeadersRow()
{
    ...
    mvec mouse_pos = ImGui::GetMousePos();
...
void ImGui::TableHeader(const char* label)
{
    ...
    mvec label_size = CalcTextSize(label, label_end, true);
    mvec label_pos = window->DC.CursorPos;
    ...
    ItemSize(mvec(0.0f, label_height)); // Don't declare unclipped width, it'll be fed ContentMaxPosHeadersIdeal
    if (!ItemAdd(bb, id))
        return;
    ...
    if ((table->Flags & ImGuiTableFlags_Sortable) && !(column->Flags & ImGuiTableColumnFlags_NoSort))
    {
        if (column->SortOrder != -1)
        {
            ...
            if (column->SortOrder > 0)
            {
                ...
                RenderText(mvec(x + g.Style.ItemInnerSpacing.x, y), sort_order_suf);
                PopStyleColor();
                x += w_sort_text;
            }
            RenderArrow(window->DrawList, mvec(x, y), GetColorU32(ImGuiCol_Text), column->SortDirection == ImGuiSortDirection_Ascending ? ImGuiDir_Up : ImGuiDir_Down, ARROW_SCALE);
        }
    ...
    //window->DrawList->AddCircleFilled(mvec(ellipsis_max, label_pos.y), 40, IM_COL32_WHITE);
    RenderTextEllipsis(window->DrawList, label_pos, mvec(ellipsis_max, label_pos.y + label_height + g.Style.FramePadding.y), ellipsis_max, ellipsis_max, label, label_end, &label_size);
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (int n = 1; n < columns->Count; n++)
        {
            ...
            const ImRect column_hit_rect(mvec(x - column_hit_hw, y1), mvec(x + column_hit_hw, y2));
            KeepAliveID(column_id);
            ...
            window->DrawList->AddLine(mvec(xi, y1 + 1.0f), mvec(xi, y2), col);
        }
